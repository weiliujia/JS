...
//3.绑定数据在页面中显示
function  bindHtml(n) {
    //n是几，我们就循环几项，就像页面插入多少箱项
    for (var i = 0; i < n; i++){
        //每一次循环的时候我们求出的5个ul当中长度最短的哪一个
    ...
    //长度最短的那个就是索引0
    uls[0].innerHTML += `<li>
            <div style="height: ${data[num].height}">
...
//4.滑动到底部重新加载一屏
window.onscroll = function () {//触发onscroll
    //要对浏览器滚动条进行判断
...
function lazyImg(ele) {
    ...
    //开始判断
    //浏览器的高度+浏览器卷出去的高度>图片自身的高度+图片距离body的上偏移量
    if (winT+winH>imgH+imgT){
        ...
        //获取到赋值给新图片的src属性
        newImg.src = url;
        //尝试让newImg加载，如果加载成功，把这个地址还给真实的i某个图片
        newImg.onload = function () {
            ele.src = this.src;
            //可以进行一些性能优化
            newImg = null;
            ele.load = true;//给图片一个自定义属性下次再碰到这张图片的时候，直接不要再继续加载了
            ele.parentNode.style.background = 'none';//图片已经有了，就不需要loading图了，我直接去掉它就行了
            fadeIn(ele)
        }
    }
}
