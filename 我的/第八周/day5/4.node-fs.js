...
/* 
  ...
  fs.writeFile(路径， 写入的内容， 什么格式， 回调函数) //有文件覆盖    没有文件就创建文件 【fs.writeFileSync()】//同步写入
  fs.appendFile(历经， 写入的内容， 什么格式， 回调函数) //在当前文件尾部追加内容 【fs.appendFileSync()】 //同步追加
  fs.readFile(路径， 读取的格式， 回调函数) //返回值是读取后的内容 【fs.readFileSync() //同步读取】
  fs.unlink(路径，回调函数) //删除文件
  fs.copyFile(路径1，路径2【目的文件】)
...
//回调函数可以忽略
//如果没有创建完成下面代码不会走
fs.writeFileSync('./testDir/1.js', 'var a={name:"zf",age:8}', 'utf-8');
